---
title: "Sample Size Calculations - I"
description: ""
author:
  - name: Andrea Gabrio
...
$$
Z \sim N\Bigg(\frac{\mu_2-\mu_1}{\sigma \sqrt{2/n}}, 1 \Bigg),
$$
...
$$
P(\mid Z \mid > 1.96 \mid H_0)=0.05 \;\;\; \text{and} \;\;\; P(\mid Z \mid > 1.96 \mid H_1) \geq 0.9
$$
To derive the distribution of the z-test statistic under $H_0$, we can simply observe that $\mu_2-\mu_1=0$, which means $Z \sim N(0,1)$, and thus $P(\mid Z \mid > 1.96 \mid H_0)=0.05$. The entire distribution of Z under $H_0$ can be graphically obtained as shown in @fig-ztesth0.
...
#| fig.pos: 'h'
#| label: fig-ztesth0
#| fig.cap: "Distribution of Z under H0"
...
#compute ss needed under H1
z_alpha <- z_crit
z_beta <- round(qnorm(0.9),2)
n1 <- (2*sigma^2*(z_alpha + z_beta)^2)/(mud^2)

#simulate dist under H0 and H1
set.seed(1234)
z0_dist <- rnorm(10000, 0 ,1)
z1_dist <- rnorm(10000, sqrt(n1)/(2*sqrt(2)) ,1)
...
ggplot(df, aes(x=z0)) + 
  ...
  geom_vline(aes(xintercept=qnorm(1-0.975)),
            color="red", linetype="dashed", linewidth=1) +
...
which shows how the distribution of Z under $H_1$ depends on the sample size $n$. A graphical comparison of the assumed distribution of Z under $H_0$ and $H_1$ is given in @fig-ztesth1.
...
#| fig.pos: 'h'
#| label: fig-ztesth1
#| fig.cap: "Distribution of Z under H0 and H1"

#draw distribution
ggplot(df, aes(x=z0)) + 
  geom_density(linewidth=1, color="blue") +
  geom_density(aes(x=z1), linewidth=1, color="red") +
  geom_vline(aes(xintercept=qnorm(0.975)),
            color="red", linetype="dashed", linewidth=1) +
  geom_vline(aes(xintercept=qnorm(1-0.975)),
            color="red", linetype="dashed", linewidth=1) +
  xlab("Survival time (weeks)") + ylab("S(t)=P(T>t)") +
  scale_x_continuous(expand = c(0, 0), limits = c(-3.5, 6.5)) +
  scale_y_continuous(expand = c(0, 0), limits = c(0, 0.45)) +
  theme(panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(), panel.background = element_blank(), 
        axis.line = element_line(colour = "black"), plot.title = element_text(hjust = 0.5), 
...
$$
n = \frac{2\times\sigma^2\times(z_{1-\alpha/2}+z_{1-\beta})^2}{(\mu_2-\mu_1)^2},
$$
...
$$
where $\Delta=\frac{\mu_2-\mu_1}{\sigma}$ is known as the *standardised* effect size.
...
$$
n = \frac{(z_{1-\alpha/2}\sqrt{2\bar{\pi}(1-\bar{\pi})}+z_{1-\beta}\sqrt{\pi_1(1-\pi_1)+\pi_2(1-\pi_2)})^2}{(\pi_2-\pi_1)^2},
$$
...
$$
where $\Delta=\frac{(\pi_2-\pi_1)}{\sqrt{(\bar{\pi}(1-\bar{\pi}))}}$ is the *standardised* effect size. 
...
#define formulae for ssc for contunuos and binary outcomes
ssc_cont <- function(alpha, beta, sigma, mu2, mu1) {
  mud <- mu2 - mu1
  z_alpha <- qnorm(1-alpha/2)
  z_beta <- qnorm(1-beta)
  n <- (2*sigma^2*(z_alpha + z_beta)^2)/(mud^2)  
  return(n)
}

ssc_bin <- function(alpha, beta, pi2, pi1) {
  pibar <- (pi1+pi2)/2
  pid <- pi2 - pi1
  z_alpha <- qnorm(1-alpha/2)
  z_beta <- qnorm(1-beta)
  n <- (z_alpha*sqrt(2*pibar*(1-pibar))+z_beta*sqrt(pi1*(1-pi1)+pi2*(1-pi2)))^2/(pid^2)
  return(n)
}
...
$$
n_1=\frac{(r+1)\times n}{2\times r},
$$
and then derive $n_2=r \times n_1$. For example, let us assume we want to use a $1:2$ (drug:placebo) allocation, which means $r=2$. Then, we can obtain the number of patients in group 1 as $n_1=3 \times n / 4$, and those in group 2 as $n_2=n_1 \times 2$.
