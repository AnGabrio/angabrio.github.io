---
title: "A quick intro on medical statistics"
description: ""
author:
  - name: Andrea Gabrio
...
date: 2025-08-05
categories: [Quarto, R, Academia, Medical Statistics] # self-defined categories
#citation: 
...
In order to ensure that a trial has a clear and precise objective, a key concept is the distinction between outcome and exposure variables in the context of a statistical analysis:
  ...
  1. The *outcome* variable is the focus of the research, i.e. a trial whose aim is to find a treatment that can reduce cholesterol levels in a group of patients should have *cholesterol levels* as the main outcome.
  
  2. The *exposure* variables are those that may influence the outcome, i.e. type of treatment.  

In general, the aims of health studies include some form of: determining the risk factors for a given health outcome, identifying geographic patterns, describe the course of an health outcome, develop preventive measures, plan health services, provide administrative planning data. 
...
One key summary measure for binary outcomes are: 
  ...
  1. Incidence: number of cases in a specified time period
  
  2. Prevalence: number of people in a population who have a disease at a given point in time
...
Relative measure estimate the magnitude of an association between exposure and disease, indicating how much more likely the exposed group is to develop disease than the unexposed group. Often, a value of $1$ for these measures means no difference between group, while a value above/below $1$ shows an adverse/protective effect of the exposure. The most commonly used measures are:
  ...
  1. Risk ratio $= \frac{\text{risk in exposed group}}{\text{risk in unexposed group}}$

  2. Odds ratio $= \frac{\text{odds in exposed group}}{\text{odds in unexposed group}}$

  3. Rate ratio $= \frac{\text{rate in exposed group}}{\text{rate in unexposed group}}$

These measures only provide point estmiates for the quantity of interest but quantification of the uncertainty surrounding these estimates is also often of great importance. Each of them has a different way to compute these measures of uncertainty, often represented in terms of some form of $95\%$ confidence interval. Consider the generic data in tabular format shown in @tbl-ci
...
tbci_y <- c("a","b")
tbci_n <- c("c","d")
tbci_tab <- rbind(tbci_y,tbci_n)
colnames(tbci_tab) <- c("Exposure","No Exposure")
rownames(tbci_tab) <- c("Disease","No Disease")
tbci_tab
```

We may compute a $95\%$ confidence interval for a risk ratio (RR) using the following quantities:

  1. RR $= \frac{a}{(a+c)}/\frac{b}{(b+d)}$
  
  2. SE log RR $= \sqrt{\frac{1}{a}-\frac{1}{(a+c)}+\frac{1}{b}-\frac{1}{(b+d)}}$
...
We may compute a $95\%$ confidence interval for an odds ratio (OR) using the following quantities:
  ...
  1. OR $= \frac{a}{c}/\frac{b}{d}$
  
  2. SE log OR $= \sqrt{\frac{1}{a}+\frac{1}{b}+\frac{1}{c}+\frac{1}{d}}$
...
We may compute a $95\%$ confidence interval for an rate ratio using the following quantities:
  ...
  1.  $d_1 =$ number of deaths in exposed group
  
  2.  $d_0 =$ number of deaths in unexposed group
  
  3. SE log rate ratio $= \sqrt{\frac{1}{d_1}+\frac{1}{d_0}}$
...
## Summary
  ...
  a. Odds ratio, rate ratio, relative risk compare the relative likelihood of an event occurring between two groups.
  
  b. Relative risk is easier to interpret but some designs prevent its calculation.
  
  c. If the disease is rate, then the three relative measures are all very similar to each other.
  
  d. Differences in risk and rates can also be measured in absolute terms, depending on the type of study.
  
  e. It may be useful to estimate both relative and absolute measures as they convey different types of information.
